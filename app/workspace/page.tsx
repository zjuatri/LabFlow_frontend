...
export default function WorkspacePage() {
  ...
  // Copy states
  const [copyingProject, setCopyingProject] = useState<Project | null>(null); // Still used for loading state if needed, or remove if not. Let's keep for loading indication potentially, or just use `copying` boolean.
  // Actually, we can remove copyingProject if we just use local ID or the dropdown ID.
  // Let's replace 'copyingProject' with 'activeDropdown' for the menu.
  ...
  const handleCopyClick = async (source: Project, e: React.MouseEvent) => {
    ...
    if (activeTab === 'report') {
      ...
      // Direct duplicate for Cover and Template
  ...
  const performCopyToTemplate = async (source: Project) => {
    ...
    try {
      ...
      // Success - Silent or switch tab? User didn't ask to switch. 
    } catch (err) {
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 selection:bg-blue-500/20">
                  ...
                  {templates.map(t => (
                    <div
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const title = t.title; // Keep the same title
      ...
      {renaming && (
        <div
          className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] animate-in fade-in duration-200"
          onMouseDown={(e) => {
            // Only close when clicking directly on the backdrop, not when dragging text selection
