...
interface NavItemEditorProps {
    ...
    onRenameDoc: (url: string, newTitle: string) => void;
    getDocId: (url: string) => string | undefined;
}

function NavItemEditor({ item, path, level, onUpdate, onDelete, onAddFolder, onCreateDocument, onMove, onRenameDoc, getDocId }: NavItemEditorProps) {
    const [isOpen, setIsOpen] = useState(true);

    const hasChildren = item.items && item.items.length > 0;
    const isDocument = !!item.url;
    const isFolder = !isDocument; // Simplified logic: if it has URL, it's a doc. Otherwise folder.

    const docId = isDocument && item.url ? getDocId(item.url) : undefined;
    ...
    return (
        <div className="mb-0">
            ...
            >
                ...
                <input
                    ...
                    onBlur={() => {
                        if (isDocument && item.url) {
                            onRenameDoc(item.url, item.title);
                        }
                    }}
                    placeholder={isFolder ? "文件夹名称" : "文档标题"}
                ...
                {/* URL Input (Document) - Read Only - REMOVED */}
                {/* {isDocument && (
                    <input
                        type="text"
                        value={item.url || ''}
                        readOnly
                        className="flex-shrink-0 w-28 px-2 py-1 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded text-xs font-mono text-zinc-500"
                        title="文档路径"
                    />
                )} */}
                ...
                <div className="flex-shrink-0 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    {/* Edit Document (Only for documents) */}
                    {isDocument && docId && (
                        <Link
                            href={`/manage/docs/edit/${docId}`}
                    ...
                    {/* Create Document (Only for folders) */}
                    {isFolder && (
                        <button
...
export default function DocsManagePage() {
    ...
    const [createDocPath, setCreateDocPath] = useState<number[]>([]);
            ...
            <main className="max-w-6xl mx-auto px-4 pt-28 pb-20">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                    <div>
                        ...
                        <h1 className="text-3xl font-bold tracking-tight">导航与文档管理</h1>
                ...
                {isCreateModalOpen && (
                    ...
                    <ul className="text-sm text-blue-900 dark:text-blue-300 mt-2 space-y-1 ml-4 list-disc">
                        ...
                        <li>点击 <strong>Edit 图标</strong> 可以编辑文档内容</li>
                        <li>修改文档标题会自动同步更新文档名称</li>
                    </ul>
                </div>
            </main>
        </div>
