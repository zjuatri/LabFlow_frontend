...
export class InputFieldParser implements BlockParser {
    parse(lines: string[], index: number): { block: TypstBlock; endIndex: number } | null {
        const trimmed = lines[index].trim();
        if (!trimmed.includes('/*LF_INPUT:')) return null;

        const block = this.parseInputFieldBlock(trimmed);
        if (block) {
    ...
    private parseInputFieldBlock(trimmed: string): TypstBlock | null {
