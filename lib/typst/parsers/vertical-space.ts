...
export class VerticalSpaceParser implements BlockParser {
    parse(lines: string[], index: number): { block: TypstBlock; endIndex: number } | null {
        // Vertical space is usually a single line
        const trimmed = lines[index].trim();
        const block = this.parseVerticalSpaceBlock(trimmed);
        if (block) {
    ...
    private parseVerticalSpaceBlock(trimmed: string): TypstBlock | null {
